...
public:
    ...
    subject() : code("   "), name(""), credit(0), prof("  "), venue("    "),  core(0) {}
    subject(string code, string name, int credit , int core, string prof,string venue)
    {
        this->code=code;
        this->name=name;
        this->credit=credit;
...
bool isvenueavailable(string,int ,int, int);
bool islecreq(int i,int j,int k);
bool islectogather(string code,string faculty,string venue,int i,int j,int k);
void displaytableindividual(int i);
bool iselelecavailable(string faculty,string venue,int i,int j,int k);
...
bool isvenueavailable(string venue,int i,int j, int k){

    for(int i=0;i<sub.size();++i ){
        if(venue==tableatumn[i][j][k].venue)
        return false;
    }
...
bool iselelecavailable(string faculty,string venue,int i,int j,int k){
    ...
    for(int a=0;a<tableatumn[i][j][k].elective.size();++a){
        ...
        if(venue==tableatumn[i][j][k].elective[a].venue)
        return false;
...
void freeslotassign(int i){
   ...
   for(int j=0;j<5;++j){
       ...
       for(int k=0;k<5;++k){
           ...
           if(isprofavailable(weeklec[t].prof,i,j,k)&&isvenueavailable(weeklec[t].venue,i,j,k) 
                && (weeklec[t].core==0)  ||  islectogather(weeklec[t].code,weeklec[t].prof,weeklec[t].venue,i,j,k)){
           ...
           if(weeklec[t].core!=0 ){
               ...
               if(iselelecavailable(weeklec[t].prof,weeklec[t].venue,i,j,k)){
...
bool islectogather(string code,string faculty,string venue,int i,int j,int k){
    ...
    for(int i=0;i<sub.size();++i){
        if(code==tableatumn[i][j][k].code&&faculty==tableatumn[i][j][k].prof&&venue==tableatumn[i][j][k].venue){
        return true;
        }
    }
    ...
    for(int j=0;j<5;++j){
        ...
        for(int k=0;k<5;k++){
            ...
            if(tableatumn[i][j][k].core==0){
                cout<<tableatumn[i][j][k].code<<","<<tableatumn[i][j][k].prof<<","<<tableatumn[i][j][k].venue<<" || ";
            }
            if(tableatumn[i][j][k].core){
                    
                for(int s=0;s<tableatumn[i][j][k].elective.size();++s){

                    cout<<tableatumn[i][j][k].elective[s].code<<","<<tableatumn[i][j][k].elective[s].prof<<","<<tableatumn[i][j][k].elective[s].venue<<" || "<<endl;
